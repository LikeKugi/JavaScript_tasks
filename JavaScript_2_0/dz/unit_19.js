...
function t1() {
  document.querySelector(".out-1").textContent = this.innerHTML;
}
...
function t2(e) {
  document.querySelector(".out-2").textContent = `ALT: ${e.altKey}`;
}
...
function t9() {
  const sources = ["img/1.png", "img/2.png"];
  let image = this.querySelector("img");
  image.src = String(image.src).endsWith(sources[0]) ? sources[1] : sources[0];
}
...
// }
// ваше событие здесь!!!
document.querySelector(".div-13").addEventListener("mousedown", (e) => {
  e.target.classList.add("active");
});
document.querySelector(".div-13").addEventListener("mouseup", (e) => {
...
function t14() {
  document.querySelector(".div-14").addEventListener("click", addActive);
}
...
function t19() {
  ...
  this.textContent = [...this.classList].join(" ");
}
// ваше событие здесь!!!
