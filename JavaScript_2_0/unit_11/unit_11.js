...
function f10() {
  for (let i = 0; i < Math.floor(d10.length / 2); i++) {
    [d10[i], d10[d10.length - 1 - i]] = [d10[d10.length - 1 - i], d10[i]];
  }
  showArr(".out-10", d10);
}
...
function f11() {
  const num = +document.querySelector(".i-11").value;
...
function f12() {
  let index = -1;
  const num = +document.querySelector(".i-12").value;
  ...
  document.querySelector(".out-12").textContent = index;
}
...
function f15() {
  ...
  if (!d15.includes(num)) d15.push(num);
  showArr(".out-15", d15);
}
...
function f18() {
  ...
  console.log(value, d18);
  document.querySelector(".out-18").textContent = d18.includes(value);
}
