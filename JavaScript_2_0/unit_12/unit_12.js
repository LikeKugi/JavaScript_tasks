...
function f6() {
  ...
  document.querySelector(".out-6").textContent = out.join(" ");
}
...
function f11() {
  let out = [];
  for (let row of a11) out.push(row.reverse().join(" "));
...
function f14() {
  let out = [];
  a14.forEach((row) => out.push(row.length));
...
function f15() {
  let mLen = a15.reduce((a, row) => (row.length > a ? (a = row.length) : a));
  document.querySelector(".out-15").textContent = mLen;
}
