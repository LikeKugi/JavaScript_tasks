...
function f3() {
  const divEls = document.getElementsByClassName("out-3");
  [...divEls].forEach((el) => (el.textContent = 3));
}
...
function f4() {
  const divEls = document.querySelectorAll(".out-4");
  divEls.forEach((el) => (el.textContent = 4));
}
...
function f9() {
  let out = a9.sort((a, b) => b.length - a.length);
...
function f11() {
  let out = [];
  for (let key in a11) {
    if (a11[key] > 10) out.push(a11[key]);
  }
...
function f12() {
  document.querySelector(".out-12").textContent = a12.join(" ");
}
...
function f13() {
  document.querySelector(".out-13").textContent = a13.split("").join(" ");
}
...
function f14() {
  document.querySelector(".out-14").textContent = [...a14].join(" ");
}
...
function f15() {
  let divEls = document.querySelectorAll(".out-15");
