...
const f3 = () => {
  let removing = document.querySelector(".i-3").value;
  s3.delete(removing);
  console.log(s3);
};
...
const f4 = () => {
  let query = document.querySelector(".i-4").value;
  console.log(s4.has(query));
};
...
const f7 = () => {
  ...
  const checking = new Set(pass);
  console.log(checking);
  document.querySelector(".out-7").textContent =
    checking.size >= 6 && checking.size == pass.length ? 1 : 0;
};
...
const f8 = () => {
  s8.forEach((el) => (el > 5 ? ar8.push(el) : null));
  console.log(ar8);
};
...
const f10 = (out_set, elem) => {
  document.querySelector(elem).textContent = f9(out_set);
};
...
const f13 = () => {
  let out = str13.split("").reduce((a, x) => {
    a[x] ? (a[x] += 1) : (a[x] = 1);
    return a;
  }, {});
  return out;
