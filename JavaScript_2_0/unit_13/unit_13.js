...
function f2() {
  ...
  return (document.querySelector(".out-2").textContent = a2.five);
}
...
function f5(arr, block) {
  ...
  document.querySelector(block).innerHTML = out.join("<br>");
...
function f7() {
  ...
  document.querySelector(".out-7").textContent = a7[key] ? 1 : 0;
}
...
function f8() {
  ...
  document.querySelector(".out-8").textContent = a8.hasOwnProperty(key)
    ? a8[key]
    : 0;
}
...
function f9() {
  ...
  let out = [];
  for (let k in a9) {
    if (a9[k] === val) out.push(k);
  }
  console.log(out);
  document.querySelector(".out-9").textContent = out.length
    ? out.join(" ")
    : " ";
}
...
function f13() {
  let total = 0;
  for (let key in a13) {
    if (typeof a13[key] === "number") total += a13[key];
  }
  document.querySelector(".out-13").textContent = total;
}
...
function f14() {
  ...
  document.querySelector(".out-14").textContent = out.join(" ");
}
...
function f16() {
  ...
  document.querySelector(".out-16").textContent = out.join(" ");
}
...
function f17() {
  let out = [];
  for (let k in a17) {
    if (a17[k].age > 30) out.push(a17[k].name);
  }
  document.querySelector(".out-17").textContent = out.join(" ");
}
...
function f18() {
  ...
  console.log(line);
  document.querySelector(".out-18").textContent = a18.hasOwnProperty(line)
    ? a18[line].join(" ")
    : " ";
}
...
function f19() {
  ...
  for (let key in a19) {
    for (let el of a19[key]) {
      if (station === el.toLowerCase()) {
...
function f20() {
  let out = [];
  for (let line in a20) {
    for (let station of a20[line]) {
      if (station[1] === 2) out.push(station[0]);
    }
  }
  document.querySelector(".out-20").textContent = out.join(" ");
}
