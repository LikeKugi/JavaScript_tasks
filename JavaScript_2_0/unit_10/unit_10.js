...
function f13() {
  let out = [];
  ar13.forEach((el, i) => out.push(i, el));
...
function f14() {
  document.querySelector(".out-14").textContent = ar14.reverse().join(" ");
}
...
function f16() {
  ar16.forEach((el, i) => (el % 2 ? (ar16_odd[i] = el) : (ar16_even[i] = el)));
...
function f17() {
  let counter = 0;
  ar17.forEach((el) => (el > 3 ? (counter += 1) : (counter += 0)));
...
function f18() {
  let max = ar18[0];
  ar18.forEach((el) => (el > max ? (max = el) : null));
...
function f19() {
  let min = 0;
  ar19.forEach((el, i) => (el < ar19[min] ? (min = i) : null));
