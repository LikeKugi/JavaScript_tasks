...
function arraySortInfo(arr) {
    if(arr.some(el => typeof el !== 'number')) return "Некоторые элементы не являются числами";

    if (arr.every( (el, i) =>  i > 0 ? el >= arr[i - 1] : true) ) return "Массив отсортирован по возрастанию";

    if (arr.every( (el, i) =>  i > 0 ? el <= arr[i - 1] : true) ) return "Массив отсортирован по убыванию";
